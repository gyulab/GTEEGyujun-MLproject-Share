...
def prune_network(network):
    prune.l1_unstructured(network.fc1, 'weight', amount=0.001)
    prune.l1_unstructured(network.fc2, 'weight', amount=0.001)
...
for i in range(n1):
    # Generate the expression for each input, excluding weights that are effectively zero
    expressions = []
    for j, inp in enumerate(["Vgs", "Vds", "Lg", "ts", "temp"]):
        weight = weights_1[i][j]
        # Only include the term if the weight is not effectively zero
        if not (abs(weight) < 1e-6):  # Adjust the threshold
            expressions.append(f"{weight:.6f}*{inp}")
    # Include the bias if it's not effectively zero
    bias = bias_1[i]
    if abs(bias) >= 1e-6:
        expressions.append(f"{bias:.6f}")
        expression = " + ".join(expressions)
    verilog_code += f"\nh1_{i} = tanh({expression});"
...
for i in range(n2):
    expressions = []
    # Generate the expression for each connection to the current neuron
    for j in range(n1):
        weight = weights_2[i][j]
        if not (abs(weight) < 1e-6):
            expressions.append(f"{weight:.6f}*h1_{j}")
    bias = bias_2[i]
    if abs(bias) >= 1e-6:
        expressions.append(f"{bias:.6f}")
        
    expression = " + ".join(expressions)
    verilog_code += f"h2_{i} = tanh({expression});"
